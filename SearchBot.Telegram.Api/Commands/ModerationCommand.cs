...
public static class ModerationCommand
{
    public static async Task BanAsync(ITelegramBotClient botClient, SearchBotContext context, Message message, string? argument, CancellationToken cancellationToken)
    {
        ...
        const string defaultReason = "не указана";
        var reason = $"Причина: {(!string.IsNullOrEmpty(argument) ? argument : defaultReason)}";
        var defaultBanMessage = $"Вы были заблокированны администратором. {reason}";
        ...
        var replyMessageToBannedUser = replyBannedUserMsg != null ? replyBannedUserMsg.Message + reason : defaultBanMessage;
